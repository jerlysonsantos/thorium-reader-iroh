...
import { downloadBlobFromTicket, seedLocalFile } from "readium-desktop/main/iroh";
...
export function* importFromLinkService(
    ...
    if ((link.type || "").replace(/\s/g, "").split(";").includes(ContentType.IrohBlob)) {
        ...
        // Seed the downloaded file into the persistent IROH node so other
        // peers can download it while the app is running.
        yield* callTyped(() => seedLocalFile(irohPath));
